...
class App extends Component {
  ...
  deleteHistoryItem = id => {
    ...
    const filteredHitoryList = historyList.filter(
      eachItem => eachItem.id !== id,
    )
    this.setState({historyList: filteredHitoryList})
  }
  ...
  render() {
    ...
    if (historyList.length === 0 || searchResults.length === 0) {
      ...
      renderElement = searchResults.map(eachItem => (
        <HistoryItem
          listItem={eachItem}
    ...
    return (
      <div className="bg-container">
        ...
        <ul className="list-container">{renderElement}</ul>
      </div>
    )
  }
}
